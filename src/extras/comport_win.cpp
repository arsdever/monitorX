...
OrderedMap<QString, QString> ComportWindows::s_comMap;
...
ComportWindows::ComportWindows(std::string const &port_name, int baudrate, int bytesize, int stopbits, int parity, int dtrcontrol)
	: __port_name(port_name), __is_valid(false), __default_settings{0}, __opened(false)
{
...
ComportWindows::~ComportWindows()
{
	close_port();
}

bool ComportWindows::is_opened() const
{
	return __opened;
}
...
bool ComportWindows::configurate()
{
	DCB settings{0};
...
uint32_t ComportWindows::write_data(const char *data, uint32_t size)
{
	DWORD written;
...
extern "C" EXTRAS_EXPORT void addMenu(QMenuBar *menuBar)
{
	...
	ComportWindows::updateComMap();
	if (ComportWindows::s_comMap.empty())
	{
		comMenu->addAction("No devices available")->setEnabled(false);
	}
	else
	{
		QList<QString> keys = ComportWindows::s_comMap.keys();
		for (QString ComportWindows : keys)
		{
			QAction *pAction = comMenu->addAction(ComportWindows);
		}
	}
}
extern "C" EXTRAS_EXPORT ComportWindows *openPort(QString const &portName)
{
