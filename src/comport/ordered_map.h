...
bool oMHashEqualToKey(const Key& key1, const Key& key2)
{
...
bool oMHashEqualToKey(Ptr* key1, Ptr* key2)
{
...
template <typename Ptr>
bool oMHashEqualToKey(const Ptr* key1, const Ptr* key2)
{
...
public:
	...
	public:
		...
		bool operator ==(const iterator& other) const { return (qllIter == other.qllIter); }
	...
	public:
		...
		bool operator ==(const const_iterator& other) const { return (qllConstIter == other.qllConstIter); }
...
private:
	...
	public:
		bool operator ==(const OMHash& other) const
		{
			...
			if (QHash<Key, OMHashValue>::operator ==(other)) { return true; }
			...
			while (it1 != this->end())
			{
				...
				if ((v1.first != v2.first) || !oMHashEqualToKey<Key>(it1.key(), it2.key())) { return false; }
				++it1;
				++it2;
			}
			return true;
...
OrderedMap<Key, Value>::OrderedMap(std::initializer_list<std::pair<Key, Value>> list)
{
	...
	for (const_initlist_iter                                                      it = list.begin(); it != list.end();
	     ++it)
...
template <typename Key, typename Value>
OrderedMap<Key, Value>::OrderedMap(const OrderedMap<Key, Value>& other) { copy(other); }
...
template <typename Key, typename Value>
bool OrderedMap<Key, Value>::contains(const Key& key) const { return data.contains(key); }

template <typename Key, typename Value>
int OrderedMap<Key, Value>::count() const { return data.count(); }

template <typename Key, typename Value>
bool OrderedMap<Key, Value>::empty() const { return data.empty(); }
...
template <typename Key, typename Value>
bool OrderedMap<Key, Value>::isEmpty() const { return data.isEmpty(); }

template <typename Key, typename Value>
QList<Key> OrderedMap<Key, Value>::keys() const { return QList<Key>::fromStdList(insertOrder.toStdList()); }
...
template <typename Key, typename Value>
int OrderedMap<Key, Value>::size() const { return data.size(); }
...
template <typename Key, typename Value>
Value OrderedMap<Key, Value>::value(const Key& key) const { return data.value(key).first; }
...
template <typename Key, typename Value>
const Value OrderedMap<Key, Value>::operator[](const Key& key) const { return value(key); }
...
template <typename Key, typename Value>
typename OrderedMap<Key, Value>::iterator OrderedMap<Key, Value>::end() { return iterator(insertOrder.end(), &data); }
